    ...
    for line in lines:
        ...
        if len(splitted_line[14]) > 0:
            ...
            if int(splitted_line[0]) >= StartTime:
                ...
                if not G.has_node(web_object):
                    G.add_node(web_object, type="web_object", timestamp=splitted_line[0])
                if not G.has_edge(web_object, domain_name) and len(splitted_line[16])==0:
                    G.add_edge(web_object, domain_name, capacity=1.0, label=edge_label, type=edge_type , timestamp=splitted_line[0])
                if not G.has_edge(splitted_line[16], domain_name) and len(splitted_line[16])>0:
                    G.add_edge(splitted_line[16], domain_name, capacity=1.0, label=edge_label, type=edge_type , timestamp=splitted_line[0])
...
def get_relation(lines_all, IncludeExecutedEdges=True, StartTime=0):
        G_prov = nx.MultiDiGraph(name=log_file, data=True, align='vertical')
        re_list=[]
        countt=-1
        for line in lines_all:
                ...
                if len(splitted_line[1]) > 0 and len(splitted_line[2]) > 0:
                        ...
                        IP_Address = splitted_line[2] #.replace(":", "_")
                        if int(splitted_line[0]) >= StartTime:
                                if not G_prov.has_node(domain_name):
                                        G_prov.add_node(domain_name, type="domain_name", timestamp=splitted_line[0])
                                if not G_prov.has_node(IP_Address):
                                        G_prov.add_node(IP_Address, type="IP_Address", timestamp=splitted_line[0])
                                re_list[countt].append([domain_name, IP_Address])
                # web_object to domain_name (in referal)
                if len(splitted_line[15]) > 0 and not splitted_line[15].startswith("/"): #  and not splitted_line[15].startswith("/") and "/" in splitted_line[15]
                        edge_type = "web_request"
                        domain_name = splitted_line[15]
                        ...
                        if int(splitted_line[0]) >= StartTime:
                                if not G_prov.has_node(domain_name):
                                        G_prov.add_node(domain_name, type="domain_name", timestamp=splitted_line[0])
                                if not G_prov.has_node(web_object):
                                        G_prov.add_node(web_object, type="web_object", timestamp=splitted_line[0])
                                re_list[countt].append([web_object, domain_name])
                # web_object to domain_name
                if len(splitted_line[14]) > 0:
                        edge_type = "web_request"
                        domain_name = splitted_line[14]
                        ...
                        if len(splitted_line[15]) > 0:
                                ...
                                web_object0 = splitted_line[15] # .replace(":", "_")
                                if int(splitted_line[0]) >= StartTime:
                                        if not G_prov.has_node(web_object0):
                                                G_prov.add_node(web_object0, type="web_object", timestamp=splitted_line[0])
                                        if not G_prov.has_node(web_object):
                                                G_prov.add_node(web_object, type="web_object", timestamp=splitted_line[0])
                                        re_list[countt].append([web_object, web_object0])
                ...
                # POST web_object to domain_name
                        ...
                        if int(splitted_line[0]) >= StartTime:
                                ...
                                if IsValidIP:
                                        edge_type = "resolve"
                                        edge_label = edge_type + "_" + str(splitted_line[0])
                                        if not G_prov.has_node(cleaned_ip):
                                                G_prov.add_node(cleaned_ip, type="IP_Address", timestamp=splitted_line[0])
                                        re_list[countt].append([domain_name, cleaned_ip])
                ...
                if len(splitted_line[3]) > 0:
                        ...
                        if not G_prov.has_node(node_name) and not node_name == "NOPROCESSNAME" and not node_name == "NOPROCESSNAME_NOPID":
                                #print node_name
                                if int(splitted_line[0]) >= StartTime:
                                        G_prov.add_node(node_name, type="process", timestamp=splitted_line[0])
                                        if program_name.endswith("/") and not program_name.endswith("//"):
                                                program_name = program_name[:len(program_name)-1] + "//"
                                        if not program_name == "NOPROGRAMNAME":
                                                program_name = program_name.rstrip()
                                                if not G_prov.has_node(program_name):
                                                        G_prov.add_node(program_name, type="file", timestamp=splitted_line[0])
                                                if IncludeExecutedEdges:
                                                        edge_type = "executed"
                                                        edge_label = edge_type + "_" + str(0)
                                                        re_list[countt].append([node_name, program_name])
                        ...
                        if len(splitted_line[4]) > 0:
                                ...
                                if not G_prov.has_node(parent_node_name) and not parent_node_name == "NOPROCESSNAME" and not parent_node_name == "NOPROCESSNAME_NOPID":
                                        if int(splitted_line[0]) >= StartTime:
                                                G_prov.add_node(parent_node_name, type="process", timestamp=splitted_line[0])
                                                if not parent_name == "NOPROCESSNAME":
                                                        if not G_prov.has_node(parent_name):
                                                                if parent_name.endswith("/"):
                                                                        parent_name = parent_name[:len(parent_name)-1] + "//"
                                                                G_prov.add_node(parent_name, type="file", timestamp=splitted_line[0])
                                                        if IncludeExecutedEdges:
                                                                edge_type = "executed"
                                                                edge_label = edge_type + "_" + str(0)
                                                                re_list[countt].append([parent_node_name, parent_name])

                                edge_type = "fork"
                                edge_label = edge_type + "_" + str(splitted_line[0])
                                if int(splitted_line[0]) >= StartTime:
                                        re_list[countt].append([node_name, parent_node_name])# if not parent_node_name in G.successors(node_name)
                        ...
                        if len(splitted_line[8]) > 0:
                                ...
                                if not G_prov.has_node(joint_ips1) and not G_prov.has_node(joint_ips2):
                                        if int(splitted_line[0]) >= StartTime:
                                                joint_ips = "connection_" + joint_ips1
                                                G_prov.add_node(joint_ips, type="connection", timestamp=splitted_line[0])
                                else:
                                        if G_prov.has_node(joint_ips1):
                                                if int(splitted_line[0]) >= StartTime:
                                                        joint_ips = joint_ips1
                                        else:
                                                if int(splitted_line[0]) >= StartTime:
                                ...
                                if not G_prov.has_node(s_ip):
                                        if int(splitted_line[0]) >= StartTime:
                                                G_prov.add_node(s_ip, type="IP_Address", timestamp=splitted_line[0])
                                if not G_prov.has_node(d_ip):
                                        if int(splitted_line[0]) >= StartTime:
                                                G_prov.add_node(d_ip, type="IP_Address", timestamp=splitted_line[0])
                                ...
                                if int(splitted_line[0]) >= StartTime:
                                        if s_ip == hosts_ips[0]: #if s_ip == "0.0.0.0" or s_ip == "127.0.0.1" or
                                                re_list[countt].append([d_ip, node_name]) # .encode('unicode_escape')
                                                re_list[countt].append([d_ip, joint_ips]) # .encode('unicode_escape')
                                        elif d_ip == hosts_ips[0]:
                                                re_list[countt].append([s_ip, node_name]) # .encode('unicode_escape')
                                                re_list[countt].append([s_ip, joint_ips]) # .encode('unicode_escape')

                                edge_type = "connect"
                                edge_label = edge_type + "_" + str(splitted_line[0])
                                if int(splitted_line[0]) >= StartTime:
                                        re_list[countt].append([joint_ips, node_name]) # .encode('unicode_escape')
                                edge_type = "sock_send"
                                edge_label = edge_type + "_" + str(splitted_line[0])
                                sender = "session_"+s_ip+"_"+s_port
                                if not G_prov.has_node(sender):
                                        if int(splitted_line[0]) >= StartTime:
                                                G_prov.add_node(sender, type="session", timestamp=splitted_line[0], ip=s_ip, port=s_port)

                                receiver = "session_"+d_ip+"_"+d_port
                                if not G_prov.has_node(receiver):
                                        if int(splitted_line[0]) >= StartTime:
                                                G_prov.add_node(receiver, type="session", timestamp=splitted_line[0], ip=d_ip, port=d_port)

                                re_list[countt].append([receiver, sender])
                                edge_type = "bind"
                                edge_label = edge_type + "_" + str(splitted_line[0])

                                if s_ip == hosts_ips[0]: #s_ip == "0.0.0.0" or s_ip == "127.0.0.1" or
                                        re_list[countt].append([sender, node_name]) # .encode('unicode_escape')
                                        edge_type = "connected_session"
                                        edge_label = edge_type + "_" + str(splitted_line[0])
                                        re_list[countt].append([d_ip, sender]) # .encode('unicode_escape')
                                elif d_ip == hosts_ips[0]:
                                        re_list[countt].append([receiver, node_name]) # .encode('unicode_escape')
                                        edge_type = "connected_session"
                                        edge_label = edge_type + "_" + str(splitted_line[0])
                                        re_list[countt].append([s_ip, receiver]) # .encode('unicode_escape')
                        ...
                        if len(splitted_line[17]) > 0 and splitted_line[17].startswith("file_") and len(splitted_line[18]) > 0:
                                ...
                                if int(splitted_line[0]) >= StartTime:
                                        if not G_prov.has_node(file_name):
                                                if file_name.endswith("/") and not file_name.endswith("//"):
                                                        file_name = file_name[:len(file_name)-1] + "//"
                                                G_prov.add_node(file_name, type="file", timestamp=splitted_line[0])
                                ...
                                for edge_type in ["readdata", "write", "delete", "execute"]: #"readdata", "writedata"
                                        ...
                                        if edge_type in accesses and not "attribute" in accesses:
                                                ...
                                                if edge_type == "read" or edge_type == "execute": #
                                                        ...
                                                        dst_node = file_name
                                                if int(splitted_line[0]) >= StartTime:
                                                        re_list[countt].append([src_node, dst_node]) # .encode('unicode_escape')
...
def load_user_artifact(file):
        ...
        mlabels_file = ""
        ua_file = open("user_artifact.txt")
...
def abstract_sessions():
    ...
    for n in ROOT_G_subgraph.nodes():
        ...
        if not n in ROOT_G_subgraph.nodes():
            continue
        if "type" in ROOT_G_subgraph.nodes(data=True)[n].keys():
        ...
        if "ip" in ROOT_G_subgraph.nodes(data=True)[n].keys():
...
def find_logs(components,re_list):
    comp_remain=[]
    global G
    results=[]
    for i in components:
        if len(i)>1:
          ...
          comp_remain.append(i)

    print("number of independant sets")
    print(len(results))
    components=results
    if len(results)==1:
        return results
    else:
        for relations in re_list:
            flag=False
            if len(components)==1:
                break
            mark=[]
            for edge in relations:
                ...
                for kk in range(len(components)):
                    if edge[0] in components[kk]:
                        tmp_mark1=str(kk)
                    if edge[1] in components[kk]:
                        tmp_mark2=str(kk)
                if len(tmp_mark2)!=0 and len(tmp_mark1)!=0 and tmp_mark1!=tmp_mark2:
                    a=set()
                    a.update(components[int(tmp_mark1)])
                    a.update(components[int(tmp_mark2)])
                    flag=True
                    copy_components=[]
                    for kk_comp in range(len(components)):
                        if str(kk_comp)!=tmp_mark1 and str(kk_comp)!=tmp_mark2:
                            copy_components.append(components[kk_comp])
                    copy_components.append(a)
                    components=copy_components
            if flag:
                for edge in relations:
                    G.add_edge(edge[0],edge[1],type="added")
    max_size=0
...
if __name__ == '__main__':
        user_node = "" #can specify
        file="raw_log"
        user_node = load_user_artifact(file)
        load_hosts_ips(file)
        sub_nodes = [user_node]
        node_set = set(sub_nodes)
        log_file=open("filtered_log","r")
        lines=log_file.readlines()
        log_file.close()
        construct_G()
        print(G.number_of_edges())
        print("end construct")
        log_file=open("raw_log","r")
        lines_all=log_file.readlines()
        log_file.close()
        re_list=get_relation(lines_all)
        components=nx.weakly_connected_components(G)
        find_logs(components,re_list) #add shortest path to connect, add such edge to G
        print("after findout")
        print(G.number_of_nodes())
        print(G.number_of_edges())
        construct_G_subgraph()
        print("after subgraph")
        print(G_subgraph.number_of_nodes())
        print(G_subgraph.number_of_edges())
        ROOT_G_subgraph = G_subgraph
        ROOT_sub_nodes = G_subgraph.nodes()
        abstract_leaf_nodes()
        abstract_sessions()
        nx.nx_pydot.write_dot(ROOT_G_subgraph, "connected_graph.dot")
